#!/usr/bin/env python3
"""Run exact DMD and gDMD for the fluidic-pinball dataset."""
...
class Config:
    input_file: Path
    output_file: Path = Path("results/pinball_gdmd_results.npz")
    n_modes: int = 10
    n_neighbors: int = 25
    dt: float = 1.0 / 160.0
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--input", dest="input_file", type=Path, required=True, help="Input .npz generated by run_pinball_dns.py.")
...
def save_or_show_figures(results: dict[str, np.ndarray], cfg: Config) -> None:
    ...
    fig.tight_layout()
    if cfg.save_figures is not None:
        fig.savefig(cfg.save_figures / "pinball_eigenvalues.png", dpi=300)
    ...
    fig2.tight_layout()
    if cfg.save_figures is not None:
        fig2.savefig(cfg.save_figures / "pinball_mode_comparison.png", dpi=300)
    ...
    if cfg.use_tke_weighting:
        ...
        fig3.tight_layout()
        if cfg.save_figures is not None:
            fig3.savefig(cfg.save_figures / "pinball_tke_weight.png", dpi=300)
